...
struct ActivePathPanelComponents: View {
...
fileprivate struct NodeEdgeGroup: View {
    ...
    var body: some View {
        Group {
            ActivePathPanelNodePanel(index: segment.index, node: segment.node)
            ActivePathPanelEdgePanel(fromNodeId: segment.id, edge: segment.edge)
        }
        .scaleEffect(scale)
