        ...
        @ViewBuilder private var content: some View {
            if let nodeIndices = selector.nodeIndices {
                ForEach(nodeIndices, id: \.from) {
                    SubpathSelectionBounds(from: $0.from, to: $0.to)
                }
            }
        }
    }

    struct SubpathSelectionBounds: View, TracedView, ComputedSelectorHolder {
        ...
        private static let strokedWidth: Scalar = 24

        @State private var dashPhase: Scalar = 0

        @ViewBuilder private var content: some View {
            if let subpath = selector.subpath {
                let width = (Self.strokedWidth * Vector2.unitX).applying(selector.viewport.viewToWorld).dx
